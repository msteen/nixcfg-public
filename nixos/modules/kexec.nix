  ...
  kexec-boot = pkgs.writeShBin "kexec-boot" ''
    if [ -d /boot ]; then
      ...
      if ! mount -t ${bootTy} -o ${bootOpts},ro ${bootDev} /kexec-boot; then
        echo 'Could not mount the boot partition' >&2
        exit 1
      fi
    ...
    cfg=$boot/kexec
    if ! [ -d "$cfg" ]; then
      echo 'Could not find the kernel image and intial ramdisk to kexec into' >&2
      exit 1
    fi
    ...
    if ! kexec --load "$boot$(cat "$cfg/kernel")" --initrd=final-initrd.gz --command-line="$(cat "$cfg/kernel-params")"; then
      echo 'Could not load the kernel image and intial ramdisk with kexec' >&2
      exit 1
    fi
